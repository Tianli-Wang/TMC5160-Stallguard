...
/**
 * @brief Read and write a given number of bytes via SPI
 * @param transmit send data
 * @param receive receive data
...
int32_t tmc5160_readInt(TMC5160TypeDef *tmc5160, uint8_t address)
{
  ...
  uint8_t receive[5];
	data[0] = address;
//	tmc5160_readWriteArray(tmc5160->config->channel, data, 5);
  tmc5160_readWriteArray(data, receive, 5, 1);
